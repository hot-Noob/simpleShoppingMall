package com.wm.shoppingWeb.util.multiRequestBody;
...
public class MultiRequestBodyArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        // 一个bug是如果json字符串仅仅是一个true，jsonObject无法解析，因为没有键值对
        ...
        // 得不到value,解析不到则将整个json串解析为当前参数类型,先判断是否是基本包装类型
        // 这一步其实没有必要根据测试jsonObject无法解析当前整个字符串是包装类型
        if (isBasicDataTypes(parameterType)) {
